...
(function(){
  ...
  document.addEventListener("keydown", function(e){
    ...
    if(k === "ArrowUp" || k === "ArrowDown" || k === "ArrowLeft" || k === "ArrowRight"){
      e.preventDefault();
      e.stopPropagation();
      // allow left/right navigation between buttons if multiple
      // If only one button, keep focus on first.
      var focusable = m.querySelectorAll('button,[href],input,select,textarea,[tabindex]:not([tabindex="-1"])');
      if(!focusable || !focusable.length) return;
      var active = document.activeElement;
      var idx = Array.prototype.indexOf.call(focusable, active);
      if(idx < 0) idx = 0;
      if(k === "ArrowLeft") idx = Math.max(0, idx - 1);
      if(k === "ArrowRight") idx = Math.min(focusable.length - 1, idx + 1);
      focusable[idx].focus();
    }
  }, true);
})();
